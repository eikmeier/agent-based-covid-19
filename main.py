...
from Schedule import create_spaces, create_dorms, create_academic_spaces, assign_dorms, assign_agents_to_classes, \
    assign_dining_times, assign_gym, \
...
def initialize():
    ...
    for division in academic_buildings:
        for day in division:
            day_index = division.index(day)
            for time in day:
                time_index = day.index(time)
                print([day_index, time_index])
                print(time)
    dining_hall_spaces = create_spaces("DiningHall", 8, 20, [16]) # We have unused Dining Hall spaces (at time 16) because the hours are not consecutive
    gym_spaces = create_spaces("Gym")
    library_spaces = create_spaces("Library")
    social_spaces = create_spaces("SocialSpace")

    stem_office_spaces = create_spaces("Office", 10, 17, None, ['A', 'B'], "STEM")
    humanities_office_spaces = create_spaces("Office", 10, 17, None, ['A', 'B'], "Humanities")
    arts_office_spaces = create_spaces("Office", 10, 17, None, ['A', 'B'], "Arts")
            ...
            for i in range(14):
                ...
                if day_schedule[i] != day_schedule[i + 1]:
                    ...
                    if agent not in all_transit_spaces[day][i + 1].agents:  # if agent is not assigned to a transit space that they are supposed to be assigned to
                        print(agent.type + " ... NOT IN TRANSIT SPACE: " + str(day_schedule[i]) + ", " + str(
                            day_schedule[i + 1]) + " [" + str(i) + ", " + str(i + 1) + "]")
    return [dining_hall_spaces, gym_spaces, library_spaces, social_spaces, stem_office_spaces, humanities_office_spaces,
            arts_office_spaces, academic_buildings[0], academic_buildings[1], academic_buildings[2], dorms]  # Return a list containing all the spaces (to be used in update)


def observe(data):
    # Figure out the spaces where everyone got exposed
    data['space_exposures'] = {"Dining Hall": 0, "Gym": 0, "Library": 0, "Large Gatherings": 0, "Social Space": 0,
                               "Office": 0, "Academic": 0, "Dorm": 0, "Transit Space": 0, "Off-Campus": 0}
...
def update():
    ...
    for week in range(SIMULATION_LENGTH):
        for day in ['A', 'B', 'A', 'B', 'W', 'W', 'S']:
            ...
            for time_index in range(15):
                for space in spaces:
                    if "Dorm" in str(space):
                        for dorm_building in space:
                            if time_index != 8 or time_index != 22:
                                dorm_building.spread_infection_core(day_index, time_index)
                            dorm_building.spread_infection_core(day_index, time_index)
                        for double_room in doubles_dorm_times[day_index][time_index]:
                            double_room.spread_infection()

                    elif "Academic" in str(space):
                        if day == "W" or day == "S":
                            break
                        if time_index in range(2, 10):
                            for building in space[day_index][(time_index - 2) // 2]:
                                building.spread_in_space()
                    else:
                        if "DiningHall" in str(space) and time_index == 13:
                            break
                        elif "Office" in str(space) and time_index == 10:
                            break
                        if type(space[day_index][time_index]) is not list:  # exclude empty lists (when space is not open)
                            space[day_index][time_index].spread_in_space()

                all_transit_spaces[day][time_index].spread_infection_core()
            ...
            # print("Day " + day + ", Week " + str(week) + ", # of Exposed Agents: " + str(len(exposed_agents)))
            print("Day " + day + ", Week " + str(week) + ", # of Infected Agents: " + str(len(infected_agents)))
            vaccinated_infected = [agent for agent in infected_agents if agent.vaccinated == 1]
            infected_students = [agent for agent in infected_agents if agent.type != "Faculty"]
    ...
    for week in range(SIMULATION_LENGTH):
        for day in ['A', 'B', 'A', 'B', 'W', 'W', 'S']:
                ...
                if day_index < len(space):  # If the space is open on this day, then spread the infection [NOTE: This requires spaces in the array always be separated by A, B, and W in order]
                    if "Dorm" in str(space):
                        ...
                        for double_dorm_day in doubles_dorm_times:
                            for double_dorms_time in double_dorm_day:
                                for double_dorm in double_dorms_time:
            ...
            infected_students = [agent for agent in infected_agents if agent.type != "Faculty"]
            infected_faculty = [agent for agent in infected_agents if agent.type == "Faculty"]
            # print([len(infected_students), len(infected_faculty)])
            # print(len(vaccinated_infected))
    ...
    for agent in agent_list:
        ...
        if agent.type == "Faculty":
            ...
            office_timelist = []
            for day in SCHEDULE_DAYS:
                schedule = agent.schedule.get(day)
                # print(schedule)
                for i, space in enumerate(schedule):
                    if space == "Office":
                        # print(schedule.index(space))
                        office_timelist.append(i)
            print(office_timelist)

        print(agent.schedule.get("W"))
    """
